...
@section('content')
    <div class="container">
        ...
        <form method="POST" action="{{ route('cabinet.banners.create.banner.store', [$category, $region]) }}"
              enctype="multipart/form-data">
            @csrf

            <div class="form-group">
                <label for="name" class="col-form-label">Название баннера</label>
            ...
            <div class="form-group">
                <label for="limit" class="col-form-label">Сколько просмотров</label>
                <input id="limit" type="number" class="form-control{{ $errors->has('limit') ? ' is-invalid' : '' }}"
                       name="limit" value="{{ old('limit') }}" required>
                @if ($errors->has('limit'))
            ...
            <div class="form-group">
                <label for="file" class="col-form-label">Картинка</label>
            ...
            <div class="form-group">
                <button type="submit" class="btn btn-primary">Добавить рекламный баннер</button>
            </div>
        </form>
    </div>
@endsection
